#-------------------------------------------------------------------------------
# Name:       vrcDetailed.py
# Purpose:    build javascript file of data records to be displayed in detail
#             i.e. no data reduction liek with vrc.py by selecting data only
#             every minute (will result in more smooth displayed chart)
#
# Author:     Michael Gries (griemide)
# Copyright:  (c) 2016, Michael Gries
#
# History:
# 2016-04-17  first creation (derived form vrc.py)
#-------------------------------------------------------------------------------
...
sourcefilename = "vrc_2016-04-16.log"  # default
destinationfilename = "vrcDayDetailed.txt" # standard output file
ftpTransferScript = "vrc2ftpDetailed.py"
datagramSize = 67
...
value = bridgeclient()
if not DEBUG:
    value.put("scriptname","vrcDetailed.py")
...
def checkService(datagram, pos):
    ...
    if ( len(datagram) == datagramRawDataLength):
       ...
       byteHex = dataset[pos]
       print "checkService.byteHex:" + byteHex
       intHex = int(byteHex)
...
def checkHeatingTime(datagram, pos):
    ...
    if ( len(datagram) == datagramRawDataLength):
       ...
       print "checkHeatingTime.wordhex:" + wordHex
       intHex = int(wordHex)
       intMask = intHex & 3
       if (intMask != 0):
          result = True
...
def checkNextMinute(datagram, pos):
    ...
    datagramRawDataLength = 203
    if ( len(datagram) == datagramRawDataLength):
       "further code only for Minute-evaluation necessary"
       dataset = datagram.split()
       global lastMinuteHex
       minuteHex = dataset[pos]
       print "checkNextMinute.minuteHex:" + minuteHex
...
vrcProgNo = "// Python 2.7.6.1 script 'vrcDetailed.py' version %s \n" % version
vrcAuthor = "// (c) 2016, Michael Gries \n"
vrcBlank0 = "//  \n"
...
for line in fobj_in:
    ...
    if checkNextMinute(line, posMinute):
       ...
       "use filter to aggregate only 2nd record each"
       filter = count%2
       print "line.filter:%s" % filter
       "if filter > 0:"
       if True:
       ...
       countLimit = 24*60*6 
       "countLimit=8640 records i.e. 10 seconds each record"
       if count > countLimit:
	  sys.exit("Runtime Error: count limit exceeded")
...
print("\n")
vrcStatus =  "%s// %s -> %s \n" % (vrcStamp2,sourcefilename,destinationfilename)
print(vrcStatus)
vrcScript = "start transfering %s via script %s ...\n" % (destinationfilename,ftpTransferScript)
print(vrcScript)
